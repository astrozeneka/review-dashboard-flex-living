...
export const ApiProvider = ({ children }: ApiProviderProps) => {
     ...
     */
    const fetchHostawayReviews = async (offset?: number, limit?: number, status?: 'all' | 'published' | 'unpublished', channel?: string, propertyName?: string, rating?: number, sortingCriteria?: SortingCriteria): Promise<{status: string; result: Review[], hasMore: boolean}> => {
        const response = await fetch(`/api/reviews/hostaway?offset=${offset || 0}&limit=${limit || 20}&status=${status || 'all'}&channel=${channel || 'all'}&propertyName=${propertyName || ''}&rating=${rating || 0}&sortingCriteria=${sortingCriteria || 'date_desc'}`, {
            headers: {
